...
* <code style="color : Magenta">Важное уточнение: общее количество бакетов подбирается так, чтобы load factor(среднее количество слов в бакете) был в среднем 20. Это нужно для того, чтобы была возможность оптимизирования функции поиска. Если бы цель была просто ускорить, мы бы увеличили размер таблицы, для убирания списков, но задача заключается в другом.</code>
